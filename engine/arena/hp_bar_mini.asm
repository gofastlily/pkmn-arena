...
; calculates bc * 16 / de, the number of pixels the HP bar has
...
GetHPBarMiniLength:
	...
	ld [hli], a
	ld [hl], $10
	call Multiply      ; 16 * bc (hp bar is 16 pixels long)
	ld a, d
	and a
...
.maxHPSmaller256
	...
	call Divide
	ldh a, [hMultiplicand+2]
	ld e, a            ; e = bc * 16 / de (num of pixels of HP bar)
	pop hl
	and a
	ret nz
...
UpdateHPBarMini2:
	...
	push de
	push bc
	call UpdateHPBarMini_CalcHPDifference
	ld a, e
	...
	pop bc
	pop de
	call UpdateHPBarMini_CompareNewHPToOldHP
	ret z
	ld a, $ff
	jr c, .HPdecrease
	ld a, $1
...
; HP decrease
	...
	ld a, b
	ld [wHPBarNewHP+1], a
	call UpdateHPBarMini_CalcOldNewHPBarPixels
	ld a, e
...
.ok
	...
	call UpdateHPBarMini_AnimateHPBar
.noPixelDifference
	ld a, [wHPBarNewHP]
	ld [wHPBarOldHP], a
	ld a, [wHPBarNewHP+1]
...
UpdateHPBarMini_CompareNewHPToOldHP:
	ld a, d
	sub b
	ret nz
	ld a, e
...
UpdateHPBarMini_CalcHPDifference:
	ld a, d
	sub b
	jr c, .oldHPGreater
	jr z, .testLowerByte
...
; d: new pixels
; e: old pixels
UpdateHPBarMini_CalcOldNewHPBarPixels:
	push hl
	ld hl, wHPBarMaxHP
	...
	push hl
	push de
	call GetHPBarMiniLength ; calc num pixels for old HP
	ld a, e
	pop de
	pop bc
	push af
	call GetHPBarMiniLength ; calc num pixels for new HP
	pop af
	ld d, e
	ld e, a
	pop hl
